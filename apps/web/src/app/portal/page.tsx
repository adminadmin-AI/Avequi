      ...
      <Card className="w-full max-w-sm">
        <CardContent className="py-8">
          <div className="mb-6 flex flex-col items-center gap-2 text-center">
            <BrandMark size={36} />
            <h1 className="text-lg font-semibold text-slate-900">Portal do Fornecedor</h1>
            <p className="text-sm text-slate-500">Informe o token de acesso enviado por e-mail.</p>
          </div>
    ...
    <div className="min-h-screen bg-slate-50">
      <header className="sticky top-0 z-10 flex h-14 items-center justify-between border-b border-slate-200 bg-white px-6">
        <div className="flex items-center gap-2.5">
          <BrandMark size={24} />
          <span className="text-sm font-semibold text-slate-900">Portal do Fornecedor</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-sm text-slate-600">{profile?.name ?? '...'}</span>
          <button onClick={onLogout} title="Sair" className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-600">
            <LogOut size={18} />
          </button>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-5xl p-6">
        <div className="mb-5 flex items-start gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-500">
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            {posQ.isLoading ? (
              ...
              <p className="py-8 text-center text-sm text-slate-400">Nenhum pedido de compra.</p>
            ) : (
                ...
                <tbody>
                  {pos.map((po) => (
                    <tr key={po.id} className="border-b border-slate-50">
                      ...
                      <td className="py-2 text-slate-600">{po.items.length} item(ns)</td>
                      ...
                      <td className="py-2 text-slate-600">{formatDate(po.createdAt)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <Card>
          ...
          <CardContent>
            {payQ.isLoading ? (
              ...
              <p className="py-8 text-center text-sm text-slate-400">Nenhum pagamento registrado.</p>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-400">
                ...
                <tbody>
                  {payments.map((p) => (
                    <tr key={p.id} className="border-b border-slate-50">
                      ...
                      <td className="py-2 text-slate-600">{p.paidAt ? formatDate(p.paidAt) : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {(summary?.overduePayments ?? 0) > 0 && (
          <div className="mt-5 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5 text-xs text-amber-700">
