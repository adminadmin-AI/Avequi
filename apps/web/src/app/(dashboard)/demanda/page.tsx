...
export default function DemandaPage() {
  ...
  const { data: forecasts = [], isLoading } = useQuery<DemandForecast[]>({
    queryKey: ['demand', user?.companyId, filterPeriod],
    queryFn: () =>
      apiClient(`/demand?period=${filterPeriod}`, token!).then((r) => r.json()),
    enabled: !!token && tab === 'filial',
  });

  const { data: consolidated = [], isLoading: loadingConsolidated } = useQuery<ConsolidatedRow[]>({
    queryKey: ['demand-consolidated', filterPeriod],
    queryFn: () =>
      apiClient(`/demand/consolidated?period=${filterPeriod}`, token!).then((r) => r.json()),
    enabled: !!token && tab === 'consolidado',
  });
  ...
  const upsertMut = useMutation({
    mutationFn: (body: object) =>
      apiClient('/demand', token!, { method: 'POST', body: JSON.stringify(body) }),
    onSuccess: () => {
  ...
  const deleteMut = useMutation({
    mutationFn: (id: string) =>
      apiClient(`/demand/${id}`, token!, { method: 'DELETE' }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['demand'] });
      qc.invalidateQueries({ queryKey: ['demand-consolidated'] });
    },
      ...
      {showForm && (
        <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-xs text-slate-500 mb-1">Período (AAAA-MM) *</label>
              <input
                type="month"
                className="w-full border rounded-lg px-3 py-2 text-sm"
                value={form.period}
          ...
          <div>
            ...
            <input
              type="text"
              className="w-full border rounded-lg px-3 py-2 text-sm"
              placeholder="Ex: Aumento sazonal para inverno..."
              value={form.notes}
          ...
          <div className="flex justify-end gap-3">
            ...
            <button
              ...
              disabled={upsertMut.isPending || !form.productId || !form.period || !form.quantity}
                ...
                {forecasts.map((f) => (
                  <tr key={f.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-right font-semibold text-slate-800">
                      {Number(f.quantity).toLocaleString('pt-BR')}
                    </td>
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-200">
            <table className="w-full text-sm">
                ...
                {consolidated.map((row) => (
                  <tr key={`${row.productId}::${row.period}`} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-right font-bold text-blue-700 text-base">
                      {row.totalQty.toLocaleString('pt-BR')}
                    </td>
              ...
              <tfoot className="bg-slate-50">
                <tr>
                  <td colSpan={2} className="px-4 py-3 text-xs text-slate-500 font-medium">
                    {consolidated.length} produto(s)
                  </td>
                  <td className="px-4 py-3 text-right font-bold text-slate-700">
                    {consolidated.reduce((s, r) => s + r.totalQty, 0).toLocaleString('pt-BR')}
                  </td>
                  <td />
                </tr>
              </tfoot>
