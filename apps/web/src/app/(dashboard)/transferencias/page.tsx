...
export default function TransferenciasPage() {
  ...
  const { data: transfers = [], isLoading } = useQuery<StoreTransfer[]>({
    queryKey: ['transfers'],
    queryFn: () => apiClient('/transfers', token!).then((r) => r.json()),
    enabled: !!token,
  });

  const { data: warehouses = [] } = useQuery<Array<{ id: string; name: string }>>({
    queryKey: ['warehouses'],
    queryFn: () => apiClient('/warehouses', token!).then((r) => r.json()),
    enabled: !!token,
  });

  const { data: products = [] } = useQuery<Array<{ id: string; name: string; sku: string }>>({
    queryKey: ['products'],
    queryFn: () => apiClient('/products', token!).then((r) => r.json()),
    enabled: !!token,
  });
  ...
  const createMut = useMutation({
    mutationFn: (body: object) =>
      apiClient('/transfers', token!, { method: 'POST', body: JSON.stringify(body) }),
    onSuccess: () => {
  ...
  const dispatchMut = useMutation({
    mutationFn: (id: string) =>
      apiClient(`/transfers/${id}/dispatch`, token!, { method: 'PATCH' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['transfers'] }),
    onError: (err, id) => setActionError((p) => ({ ...p, [id]: (err as Error).message })),
  });

  const receiveMut = useMutation({
    mutationFn: (id: string) =>
      apiClient(`/transfers/${id}/receive`, token!, { method: 'PATCH' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['transfers'] }),
    onError: (err, id) => setActionError((p) => ({ ...p, [id]: (err as Error).message })),
  });
  ...
  const cancelMut = useMutation({
    mutationFn: (id: string) =>
      apiClient(`/transfers/${id}/cancel`, token!, { method: 'PATCH' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['transfers'] }),
    onError: (err, id) => setActionError((p) => ({ ...p, [id]: (err as Error).message })),
  });
      ...
      {showForm && (
        <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-slate-500 mb-1">Origem (fábrica/depósito) *</label>
              <select
            ...
            <div>
              <label className="block text-xs text-slate-500 mb-1">Destino (loja) *</label>
              <select
                className="w-full border rounded-lg px-3 py-2 text-sm"
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button className="text-xs text-blue-600 hover:underline" onClick={addItem}>+ Adicionar item</button>
            </div>
              ...
              <div key={idx} className="flex gap-3 items-center">
                <select
                  ...
                  onChange={(e) =>
                    setForm((p) => ({
                      ...p,
                      items: p.items.map((it, i) => i === idx ? { ...it, productId: e.target.value } : it),
                    }))
                  }
                >
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((p) => ({
                      ...p,
                      items: p.items.map((it, i) => i === idx ? { ...it, quantity: Number(e.target.value) } : it),
                    }))
                  }
                />
                {form.items.length > 1 && (
        ...
        <div className="space-y-3">
          {transfers.map((tr) => (
            <div key={tr.id} className="bg-white border border-slate-200 rounded-xl p-4">
                  ...
                  {tr.fiscalDocument && (
                    <div className="text-xs text-slate-500">
                      ...
                      <span className={`px-1 rounded text-xs ${
                        tr.fiscalDocument.status === 'AUTHORIZED' ? 'bg-green-100 text-green-700' :
                        tr.fiscalDocument.status === 'REJECTED' ? 'bg-red-100 text-red-700' :
                        'bg-yellow-100 text-yellow-700'
                      }`}>{tr.fiscalDocument.status}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-2 flex-shrink-0">
                  {tr.status === 'DRAFT' && (
                    <>
                      <button
                        ...
                        onClick={() => dispatchMut.mutate(tr.id)}
                      ...
                      <button
                        ...
                        onClick={() => cancelMut.mutate(tr.id)}
                  ...
                  {tr.status === 'DISPATCHED' && (
                    <span className="text-xs text-slate-400 self-center">
                      despachado {formatDate(tr.dispatchedAt)}
                    </span>
                  )}
                  {tr.status === 'RECEIVED' && (
                    <span className="text-xs text-green-600 self-center">
                      recebido {formatDate(tr.receivedAt)}
                    </span>
                  )}
                </div>
              </div>
              ...
              {actionError[tr.id] && (
                <p className="text-xs text-red-500 mt-2">{actionError[tr.id]}</p>
              )}
            </div>
          ))}
        </div>
