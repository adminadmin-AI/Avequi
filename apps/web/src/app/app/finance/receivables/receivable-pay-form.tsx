...
export function ReceivablePayForm({
  formId,
  remaining,
  onSubmit,
}: {
  ...
  return (
    <form id={formId} onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-1">
      ...
      <div className="grid grid-cols-2 gap-4">
        <Field label="Data do recebimento" required error={errors.paidAt?.message}>
          <Input type="date" {...register('paidAt')} error={!!errors.paidAt} />
        </Field>
        <Field label="Valor recebido" required error={errors.paidAmount?.message}>
          <Input
            type="number"
            step="0.01"
            min="0.01"
