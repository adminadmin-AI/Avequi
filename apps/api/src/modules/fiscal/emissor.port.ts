...
export interface EmissorPort {
  ...
  emitNFe(ref: string, payload: Record<string, unknown>): Promise<EmissionResponse>;

  /** Emitir NFC-e (modelo 65) */
  emitNFCe(ref: string, payload: Record<string, unknown>): Promise<EmissionResponse>;

  /** Cancelar NF-e/NFC-e autorizada (prazo SEFAZ 24h) */
  cancelNFe(ref: string, justificativa: string): Promise<EmissionResponse>;

  /** Carta de Correção eletrônica */
  sendCCe(ref: string, correcao: string): Promise<EmissionResponse>;
  ...
  fetchReceivedNfes(cnpj: string): Promise<any[]>;
  ...
  getStatus(type: 'nfe' | 'nfce', ref: string): Promise<EmissionResponse>;
}
