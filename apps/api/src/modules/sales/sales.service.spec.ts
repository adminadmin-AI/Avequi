...
describe('SalesService', () => {
  ...
  describe('createOrder — credit check (#187)', () => {
    it('OV acima do limite → CREDIT_HOLD', async () => {
      // Customer has credit limit of 300, already used 200, order total = 500 (5 × 100)
      ...
      // creditAvailable = 300 - 200 = 100 < 500 → CREDIT_HOLD
      const createdOrder = {
        ...baseOrder,
        status: 'CREDIT_HOLD',
        customerId: 'cust-1',
      ...
      mockPrisma.salesOrder.create.mockResolvedValue(createdOrder);
      mockPrisma.auditLog.create.mockResolvedValue({});
      ...
      expect(result.status).toBe('CREDIT_HOLD');
      expect(mockPrisma.salesOrder.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ status: 'CREDIT_HOLD' }),
        }),
      );
    });

    it('OV dentro do limite → DRAFT (normal)', async () => {
      mockPrisma.creditLimit.findFirst.mockResolvedValue({
        id: 'cl-1',
        maxAmount: 1000,
        status: 'ACTIVE',
    ...
    it('cliente sem limite configurado → DRAFT (sem restrição)', async () => {
  ...
  describe('approveCreditHold (#187)', () => {
    it('CREDIT_HOLD → DRAFT após aprovação gerencial', async () => {
      mockPrisma.salesOrder.findFirst.mockResolvedValue({
        ...baseOrder,
        status: 'CREDIT_HOLD',
      });
      ...
      mockPrisma.salesOrder.update.mockResolvedValue(updatedOrder);
      mockPrisma.auditLog.create.mockResolvedValue({});

      const result = await service.approveCreditHold('so-1', 'co-1', 'user-1');
      expect(result.status).toBe(SalesOrderStatus.DRAFT);
    });

    it('deve rejeitar aprovação de OV que não está em CREDIT_HOLD', async () => {
      mockPrisma.salesOrder.findFirst.mockResolvedValue(baseOrder); // DRAFT
      await expect(
        service.approveCreditHold('so-1', 'co-1', 'user-1'),
      ).rejects.toThrow(BadRequestException);
    });
  });
});
