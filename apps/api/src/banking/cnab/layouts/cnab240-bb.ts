...
// Banco do Brasil — Código 001, ISPB 00000000
export class Cnab240BB extends Cnab240Base {
  protected bankCode = '001';
  protected bankName = 'BANCO DO BRASIL S.A.';

  private readonly ISPB = '00000000';

  // ─── Occurrence codes for BB return files ────────────────────────────────
  ...
  private static readonly OCCURRENCES: Record<string, string> = {
    ...
    '21': 'Acerto do Controle do Número do Título',
  ...
  generateSegmentoP(boleto: CnabBoleto, loteSeq: number, itemSeq: number): string {
    ...
    line += this.padLeft(boleto.nossoNumero, 20);      // 038-057  Nosso número (BB: convênio+nossoNr)
    line += '17';                                       // 058-059  Carteira BB (17=cobrança registrada)
    line += '1';                                        // 060      Forma de cadastro
    ...
    line += '00';                                       // 120-121  Instrução 2 (00=sem)
  ...
  generateSegmentoR(boleto: CnabBoleto, loteSeq: number, itemSeq: number): string {
    ...
    line += ' ';                                        // 015
    line += '01';                                       // 016-017
    line += '0';                                        // 018      Desconto 2
    line += this.padLeft('0', 8);                      // 019-026
    line += this.formatDecimal(0, 15, 2);              // 027-041
    line += '0';                                        // 042      Desconto 3
    line += this.padLeft('0', 8);                      // 043-050
    line += this.formatDecimal(0, 15, 2);              // 051-065
    line += '2';                                        // 066      Multa - código (2=percentual)
    line += this.padLeft('0', 8);                      // 067-074  Data multa
    line += this.formatDecimal(2, 15, 2);              // 075-089  Percentual multa
  ...
  parseRetornoLine(line: string): RetornoItem | null {
    ...
    const occurrenceDesc = Cnab240BB.OCCURRENCES[occurrence] ?? `Ocorrência ${occurrence}`;
