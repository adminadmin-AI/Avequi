...
> Épico **#583** · issues **#584–#590** · board [projects/7](https://github.com/users/adminadmin-AI/projects/7/views/1)
...
## 4. Desenho (boa prática)

- **(a)** Modelo `SalesPayment` — plano de pagamento por venda (1+ formas; `detPag` é lista).
- **(b)** Cadastro de adquirentes + tabela de taxas **MDR** por bandeira/modalidade/parcelas + **prazo de liquidação**.
- **(c)** Refatorar a geração de títulos no faturamento para ler o plano (boleto→N parcelas por prazo contra o cliente; cartão→N recebíveis contra a adquirente, líquidos, D+n; MDR como despesa). Corrige o bug do "30 dias fixo".
- **(d)** Conciliação de cartão (arquivo/API da adquirente; baixa dos títulos; taxa divergente, antecipação, chargeback).
- **(e)** Fiscal: grupo `card` na NF-e (tPag 03/04, `tpIntegra`, `tBand`, `cAut`). `tpIntegra=2` (maquininha não integrada) dispensa detalhes — começa simples.

## 5. Faseamento por prioridade

| Fase | Issues | Entrega | Por quê |
|------|--------|---------|---------|
| **1 — MVP** | #584 (a) · #585 (b) · #586 (c) · #587 (e) | Plano de pagamento + adquirente/taxa + recebível líquido com data certa + MDR como despesa (baixa manual) | Maior retorno — cartão é a maioria; corrige a distorção de caixa e o bug dos 30 dias fixos |
| **2 — Automação** | #588 (d) | Conciliação automática, antecipação, chargeback | Tira o trabalho manual quando o volume justificar |
| **3 — Boleto/PIX/CNAB** | #589 · #590 | Boleto/PIX real via API + remessa CNAB240 | Boleto é pouco volume — pode esperar |

## 6. Inputs pendentes do Claudio

- Adquirentes/bandeiras que a GDR usa e as taxas contratadas (seed da issue #585).
- Maquininha é **integrada** ao ERP (TEF) ou não? Define `tpIntegra` (#587) e o nível de automação da conciliação (#588).
